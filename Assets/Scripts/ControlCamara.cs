...
public class ControlCamara : MonoBehaviour {
	...
	void Update () {
		...
		case Central.EstadoJuego.EnJuego:
			transform.position = new Vector3 (Mathf.Lerp(transform.position.x, zanahoria.position.x + 30f, 1f * Time.deltaTime), Mathf.Lerp(transform.position.y, 0f, 1f * Time.deltaTime), transform.position.z);
			camara.orthographicSize = Mathf.Lerp(camara.orthographicSize, 30f + (0.5f - (Screen.width / Screen.height - 1.25f) * 20f), 1f * Time.deltaTime);
			break;
		}
	}
}
